  ...
  context 'MultipleAccessor' do
    ...
    context '#read' do
      shared_examples_for 'reading a multiple enum' do
        it 'returns SimpleEnum::CollectionProxy' do
          expect(subject.read(object)).to be_a SimpleEnum::Multiple::CollectionProxy
        end
    ...
    shared_examples_for '#scope' do
      it 'returns an ActiveRecord::Relation' do
        ...
        expect(klass.ipads).to be_a(ActiveRecord::Relation)
      end
    end

    context 'bitwise accessor', active_record: true do
      fake_active_record(:klass) { as_enum :favorites, [:iphone, :ipad, :macbook], accessor: :bitwise }
